import type { TacticalSentinelEvaluation, TacticalSentinelStatus } from "@shared/types/horizon";
import { TACTICAL_THRESHOLDS } from "@shared/constants/thresholds";
import type { EvaluationDetail } from "@shared/types/horizon";

export function calculateTacticalScore(details: Record<string, EvaluationDetail>): number {
  const totalScore = Object.values(details).reduce((sum, detail) => sum + detail.score, 0);
  return Math.min(100, Math.max(0, totalScore));
}

export function determineTacticalStatus(score: number): TacticalSentinelStatus {
  if (score >= TACTICAL_THRESHOLDS.TRADE_MIN_SCORE) {
    return "TRADE";
  }
  if (score >= TACTICAL_THRESHOLDS.WATCH_MIN_SCORE) {
    return "WATCH";
  }
  return "AVOID";
}

export function extractEntryQuality(details: Record<string, EvaluationDetail>): string[] {
...
export function extractTacticalRisks(details: Record<string, EvaluationDetail>): string[] {
  const risks: string[] = [];
...
export function determineFailureTrigger(details: Record<string, EvaluationDetail>): string {
...
export function buildTacticalEvaluation(
  ...
  const status = determineTacticalStatus(score);
